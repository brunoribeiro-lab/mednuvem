...
class Encryption extends ServiceProvider {
     ...
     */
    public static function encrypt($plain, $Key = NULL, $Salt = NULL) {
        ...
        if (empty($Salt)) {
            $Salt = config('app.cipher');
        }
        // generate key for authentication using ENCRYPTION_KEY & HMAC_SALT
        $key = mb_substr(hash(self::HASH_FUNCTION, $Key . $Salt), 0, 32, '8bit');
     ...
     */
    public static function decrypt($ciphertext, $Key = NULL, $Salt = NULL) {
        if (empty($ciphertext)) {
